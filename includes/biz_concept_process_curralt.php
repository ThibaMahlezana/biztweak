...
if (isset($_POST['bizconceptcurralt-update'])) {
...
$b = mysqli_real_escape_string($conn, $_POST['radio1']);
	...
	exit ();
		...
		$sql = "SELECT sdatauid FROM bizConCurrAlt WHERE sdatauid=?";
		...
		header("Location: ../biz_concept.php?error=sqlerror");
			...
			$sql = "INSERT INTO bizConCurrAlt (sdatausersid, sdatauid, sdataq115, sdataq116) VALUES (?,?,?,?)";
				$stmt = mysqli_stmt_init($conn);
		if (!mysqli_stmt_prepare($stmt, $sql)) {
		header("Location: ../biz_concept.php?error=sqlerror");
			exit();
