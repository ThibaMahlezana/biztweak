...
if (isset($_POST['bizconceptunfad-update'])) {
...
$a = mysqli_real_escape_string($conn, $_POST['radio0']);
	...
	exit ();
		...
		$sql = "SELECT sdatauid FROM bizConUnfAd WHERE sdatauid=?";
		...
		header("Location: ../biz_concept.php?error=sqlerror");
			...
			$sql = "INSERT INTO bizConUnfAd (sdatausersid, sdatauid, sdataq118) VALUES (?,?,?)";
				$stmt = mysqli_stmt_init($conn);
		if (!mysqli_stmt_prepare($stmt, $sql)) {
		header("Location: ../biz_concept.php?error=sqlerror");
			exit();
