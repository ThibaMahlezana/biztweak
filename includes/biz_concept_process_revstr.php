...
if (isset($_POST['bizconceptrevstr-update'])) {
...
$d = mysqli_real_escape_string($conn, $_POST['radio3']);
	...
	exit ();
		...
		$sql = "SELECT sdatauid FROM bizConRevStr WHERE sdatauid=?";
		...
		header("Location: ../biz_concept.php?error=sqlerror");
			...
			$sql = "INSERT INTO bizConRevStr (sdatausersid, sdatauid, sdataq103, sdataq104, sdataq105, sdataq106) VALUES (?,?,?,?,?,?)";
				$stmt = mysqli_stmt_init($conn);
		if (!mysqli_stmt_prepare($stmt, $sql)) {
		header("Location: ../biz_concept.php?error=sqlerror");
			exit();
