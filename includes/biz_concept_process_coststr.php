...
$d = mysqli_real_escape_string($conn, $_POST['radio3']);
	...
	exit ();
		...
		$sql = "SELECT sdatauid FROM bizConCostStr WHERE sdatauid=?";
		...
		header("Location: ../biz_concept.php?error=sqlerror");
			...
			$sql = "INSERT INTO bizConCostStr (sdatausersid, sdatauid, sdataq111, sdataq112, sdataq113, sdataq114) VALUES (?,?,?,?,?,?)";
				$stmt = mysqli_stmt_init($conn);
		if (!mysqli_stmt_prepare($stmt, $sql)) {
		header("Location: ../biz_concept.php?error=sqlerror");
			exit();
