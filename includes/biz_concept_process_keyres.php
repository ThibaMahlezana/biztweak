...
if (isset($_POST['bizconceptkeyres-update'])) {
...
$a = mysqli_real_escape_string($conn, $_POST['radio0']);
	...
	exit ();
		...
		$sql = "SELECT sdatauid FROM bizConKeyRes WHERE sdatauid=?";
		...
		header("Location: ../biz_concept.php?error=sqlerror");
			...
			$sql = "INSERT INTO bizConKeyRes (sdatausersid, sdatauid, sdataq108) VALUES (?,?,?)";
				$stmt = mysqli_stmt_init($conn);
		if (!mysqli_stmt_prepare($stmt, $sql)) {
		header("Location: ../biz_concept.php?error=sqlerror");
			exit();
